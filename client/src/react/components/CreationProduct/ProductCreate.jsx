...
function validate(input) {
	...
	if (!input.name || nameVal.test(input.name) === false)
		errors.name = <i>"Debe ingresar un nombre válido para el producto"</i>;
	if (!input.description || nameVal.test(input.description) === false)
	...
	if (
		...
		isNaN(Number(input.currentPrice))
	) {
		errors.currentPrice = <i>"Debe ingresar un importe valido"</i>;
	}

	if (!input.color || lettersVal.test(input.color) === false) {
		errors.color = <i>"Debe ingresar un color de prenda valido"</i>;
	}

	if (!input.gender) errors.gender = <i>"Debe ingresar un genero"</i>;
	if (!input.brandName) errors.brandName = <i>"Debe ingresar una marca"</i>;
	if (!input.category) errors.category = <i>"Debe ingresar una marca"</i>;
...
export default function ProductCreate() {
	...
	function handleSubmit(e) {
		...
		if (Object.values(errors).length !== 0) {
			Swal.fire("Faltan campos que rellenar!", "", "warning");
		} else {

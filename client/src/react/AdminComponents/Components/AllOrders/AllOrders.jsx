...
export default function AllClients() {
	...
	function handleModifiedStatus(e, payment_id) {
		...
		if (e.target.value === "failure") {
			const { orderDetails } = allOrdersClientes.find(
				(el) => Number(el.payment_id) === Number(payment_id),
			);

			dispatch(addStock(orderDetails));
		}
		...
		<div className={style.cardContainer}>
			...
			<div className={style.container}>
				...
				{allOrdersClientes?.map((client) => {
					return (
						<div className={style.orders} key={client.payment_id}>
							<NavLink
